...
@Service
public class SocketServerSelector {
    ...
    public void init() {
        try {
            logger.info("Server Host " + hsServiceProperties.getSocketAddress() + " port " + hsServiceProperties.getSocketPort());
            host = hsServiceProperties.getSocketAddress();
            port = hsServiceProperties.getSocketPort();
            linkManger.addDeviceListener(heartBeatService.getConnectListener());
            initSelector();
        } catch (IOException e) {
            logger.error("init SocketService error", e);
        }
    }

    @PreDestroy
    public void shutdown() {
        logger.info("Shutting down SocketServerSelector");
        executor.shutdown();
    }
